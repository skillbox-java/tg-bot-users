...
from database.commands import temp_cleaner, select_last_uncongratulate, get_all_moderator_id, select_group_id, \
    ...
    storage_cleaner_unceleb, record_cleaner_unceleb
import datetime
...
def bot_uncongratulate(message: Message) -> None:
    ...
    if message.chat.id in moderator_ids:
        ...
        uncelebs = select_last_uncongratulate(moderator_id=message.chat.id)
        gr_id = select_group_id(message.chat.id)
        temp_id = 0
        uncelebs_list = []
...
def callback(call):
    if call.message:
        if call.data == 'congr':
            ...
            winner = is_winner_id_select_unceleb(bot_message_id=call.message.message_id)
            # print(winner)
